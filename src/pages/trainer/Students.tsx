...
export default function TrainerStudents() {
  ...
  const filteredStudents = displayStudents.filter((student) => {
    ...
    // 'inactive' -> show none
    // 'all' -> show all
    const matchesFilter = filter === 'all' ? true : filter === 'active' ? true : false; // filter === "inactive"
    ...
    <div className="min-h-screen p-6 lg:p-8">
      ...
      <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <h1 className="font-display text-2xl font-bold text-foreground lg:text-3xl">Alunos</h1>
            <p className="mt-1 text-muted-foreground">
              Gerencie seus {filteredStudents.length} alunos
            </p>
          </div>
          <Button variant="default" size="lg" onClick={() => navigate('/trainer/students/new')}>
                ...
                <CardContent className="p-5">
                  ...
                  <div className="mb-4 flex items-start justify-between">
                    <div className="flex items-center gap-3">
                      <div className="bg-gradient-primary flex h-12 w-12 items-center justify-center rounded-full font-semibold text-primary-foreground">
                        {student.name
                          .split(' ')
                          .map((n) => n[0])
                          .join('')
